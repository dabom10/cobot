...
class IntegratedSystem:
    ...
    def shaking_process(self, idx, base_progress):
        ...
        wait(6)
        movel(posx([0,0,30,0,0,0]), vel=VELX, acc=ACCX, mod=DR_MV_MOD_REL)
        ...
        # [STEP 1] 시작 위치 도달 및 세로 쉐이킹
        movej(J_SHAKE_START, vel=VELJ, acc=ACCJ)
        start_j_normal = get_current_posj()    
        
        print("1. 수직 쉐이킹 시작")
        for i in range(ITER_COUNT):
            # J4는 고정하고 J3, J5, J6 오프셋으로 촥촥 흔듦
            target_up = get_safe_joint(start_j_normal, [0, 0, -AMP_J3, 0, -AMP_J5, -AMP_J6])
            target_down = get_safe_joint(start_j_normal, [0, 0, AMP_J3, 0, AMP_J5, AMP_J6])
            
            movej(target_up, vel=V_SHAKE, acc=A_SHAKE, radius=5)
            movej(target_down, vel=V_SHAKE, acc=A_SHAKE, radius=5)

        # [STEP 2] 가로 쉐이킹 (J1의 좌우 이동과 J4/J6의 스냅 결합)
        print("2. 가로 쉐이킹 시작")
        
        for i in range(ITER_COUNT):
            # J1을 흔들어 엔드이펙터가 실제로 좌우로 크게 이동하게 하고,
            # J4와 J6를 교차로 흔들어 세로 쉐이킹과 같은 '촥촥' 소리가 나는 역동적인 스냅을 줍니다.
            target_left = get_safe_joint(start_j_normal, [-AMP_J1, 0, 0, -AMP_J4, 0, AMP_J6])
            target_right = get_safe_joint(start_j_normal, [AMP_J1, 0, 0, AMP_J4, 0, -AMP_J6])
            
            movej(target_left, vel=V_SHAKE, acc=A_SHAKE, radius=5)
            movej(target_right, vel=V_SHAKE, acc=A_SHAKE, radius=5)

        # 마무리: 원래 위치로 복구
        movej(start_j_normal, vel=VELJ, acc=ACCJ)
        print("모든 쉐이킹 동작 완료")
